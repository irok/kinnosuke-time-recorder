...
var KTR = (function(){
	...
	var Cache = (function(){
		...
		return {
			get: function(key) {
				try {
					var cache = JSON.parse(store[key]);
	...
	KTR.notify = (function(){
		var count = 0;
		var defaultOptions = {
				type:'basic', title:'情報', duration:3500,
				iconUrl:chrome.runtime.getManifest().icons['128']
		};
		var clear = function(duration, id) {
			setTimeout(function(){ chrome.notifications.clear(id, NOP) }, duration);
		};

		// public interface
		return function(msg, opts) {
			var notifyId = 'KTR-notify' + (++count);
			...
			chrome.notifications.create(
				notifyId, options,
				duration > 0 ? clear.bind(this, duration) : NOP
			);
		};
	})();
	...
	KTR.status = {
		...
		apply: function(html) {
			var status = KTR.status.analyze(html);
	...
	function status_cache() {
		...
		if (arguments[0] === null) {
			...
			Cache.set('status', arguments[0], 10 * 60);
		}
	}
